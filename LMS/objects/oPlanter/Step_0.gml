...
if (state == planter.PICKING) {
	...
	if (selection_position > (struct_names_count(seeds) - 1)) and (mouse_wheel_up() or gamepad_button_check_pressed(player_device, gp_padu)) {
	...
	if (selection_position == -1) and (mouse_wheel_down() or gamepad_button_check_pressed(player_device, gp_padd)) {
		selection_position = struct_names_count(seeds) - 1
	}
}

if (state == planter.PICKING) and (mouse_check_button_pressed(mb_right) or (gamepad_button_check(player_device, gp_face2))) {
	state = planter.EMPTY;
